...
But we want to not only disconnect living mapped drives but also remove "ghost" drives, which are not accessible any longer (they have a red cross like they would disonnected in Wondows Explorer but you cannot open or remove them). That could have different reasons: One is, that the drive was created under SYSTEM rights (like I said, dont to this). Another cause is that the service LanmanWorkstation was performed without proper net use delete before. In rare cases it happens just because Windows is stupid and is full of bugs (yes, sometimes this is not caused by a user). There were two obstacles to fiddle with: How can I filter for ghost drives only to avoid removing normal mapped drives? And then, how can I actually remove the ghost drives? I tried various onboard tools and crawled the entire registry for remnants - nothing, no trace. Net use was no help since it said there weren't any mappings anymore, same with net session. Diskpart does not differ between working/dead mappings too.
At the end I created a powershell approach that is indeed able to list only dead drives cause they are lack of an actual provider/path:<br>
*Get-WmiObject Win32_LogicalDisk -Filter "DriveType = 4" | Where-Object { [string]::IsNullOrEmpty($_.ProviderName) }* <br>
As for the removal itself: I found mountvol.exe very interesting, cause it is able to remove an assign letter completely. Because the dead mapping drives does not have any content, here is nothing to loose. Unfortunately since Windows 10, mountvol.exe needs even elevated rights to unmount user based mappings, but I cannot change that fact and it's not a problem since my tool needs elevated rights anyway.
